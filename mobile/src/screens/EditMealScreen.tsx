  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (image && image !== meal?.image && image.startsWith('file:')) {
        // Only upload if changed AND it's a local file
        // @ts-ignore
...
const styles = StyleSheet.create({
  container: { padding: 20, backgroundColor: Colors.background, minHeight: 600 },
  ...
  delBtn: { marginTop: 12, backgroundColor: Colors.danger, padding: 14, borderRadius: 8, alignItems: 'center' },
  delBtnText: { color: '#fff', fontWeight: 'bold', fontSize: 18 },
});
