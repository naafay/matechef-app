...
export default function MyMealsScreen() {
  ...
  // This navigation hook is **inside** the MyMeals stack.
  const navigation = useNavigation<any>();

  // Redirect to ChefProfileSetupScreen if address is not set
  useEffect(() => {
    if (!user?.address) {
      navigation.replace('ChefSetup');
    }
  }, [user?.address]);
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={styles.empty}>No meals yet. Tap “Add Meal” to start.</Text>
        }
        renderItem={({ item }) => (
          <TouchableOpacity
            style={styles.card}
            onPress={() => {
              // We confirm that this navigation is **inside** MyMeals stack:
              console.log(
                '[MyMealsScreen] Opening EditMeal under MyMeals tab. Parent routes:',
                JSON.stringify(navigation.getParent()?.getState().routeNames)
              );
              navigation.navigate('EditMeal', { meal: item });
            }}
          >
